...
export const problems: Problem[] = [
  ...
  {
    problem_id: "Q02",
    title: "Second Highest Order Value",
    pattern: "Ranking / subquery / limit-offset",
    schema: "ecommerce",
    query:
  ...
  },
  {
    ...
    schema: "ecommerce",
    query:
      "SELECT * FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY category_id ORDER BY price DESC) AS rank FROM products) t WHERE rank <= 3;",
  },
  {
    problem_id: "Q05",
    title: "Customers Who Never Ordered",
    pattern: "LEFT JOIN + NULL filter",
    schema: "ecommerce",
    query:
  ...
  },
  {
    ...
    schema: "ecommerce",
    query:
      "SELECT order_id, COUNT(product_id) as item_count FROM order_items GROUP BY order_id HAVING COUNT(product_id) > 1;",
  },
  {
    problem_id: "Q07",
  ...
  },
  {
    ...
    schema: "ecommerce",
    query:
      "SELECT c.customer_id, SUM(o.total_amount) AS total_revenue FROM customers c JOIN orders o ON c.customer_id = o.customer_id GROUP BY c.customer_id;",
  },
];

export function getProblemById(id: string): Problem | undefined {
  return problems.find((p) => p.problem_id === id);
}

export const patternColors: Record<string, string> = {
  "GROUP BY + HAVING": "#F97316",
  "Ranking / subquery / limit-offset": "#8B5CF6",
  Subquery: "#3B82F6",
  "Window functions": "#10B981",
  "LEFT JOIN + NULL filter": "#EC4899",
  "Multi-table joins + aggregation": "#F59E0B",
};
